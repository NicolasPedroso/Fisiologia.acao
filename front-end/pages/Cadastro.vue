<template>
	<v-container fluid class="ma-0 pa-0">
		<v-row>
			<v-col cols="11">
				<v-row>
					<v-col cols="6" class="d-flex justify-start align-start" style="
							height: 250px;
							padding-left: 80px;
							padding-top: 50px;
						">
					...
					<v-col cols="6" class="d-flex justify-center align-center flex-column"
						...
						<v-file-input v-model="file" accept="image/*" label="Escolha uma imagem"
							prepend-icon="mdi-camera-plus-outline" color="black" hide-input rounded
							:rules="[imageRequired]" class="custom-icon-size" :error-messages="fileError"
							@change="previewImage">
						</v-file-input>
						<span class="text-error">Coloque uma foto de perfil. (obrigatório)</span>
					</v-col>
				</v-row>
			</v-col>
			...
			<v-col cols="11" class="d-flex justify-center" style="margin-bottom: 30px">
				<v-col cols="7">
					<v-card variant="outlined" style="padding: 40px">
						<v-col cols="12" class="d-flex justify-center font-weight-black text-blue-darken-4 text-h4">
						...
						<v-col cols="12">
							<span>TELEFONE</span>
							<v-text-field v-model="phone" label="(XX) XXXXXXXX" variant="outlined"
								prepend-inner-icon="mdi-phone-outline" clearable required :rules="[phoneRules]"
								@input="formatPhone"></v-text-field>
						</v-col>

						<v-col cols="12">
							<span>ENDEREÇO</span>
							<v-text-field v-model="address" label="Coloque seu endereço (opcional)" variant="outlined"
								prepend-inner-icon="mdi-home-outline" clearable></v-text-field>
						</v-col>
						...
						<v-col cols="12">
							...
							<v-text-field v-model="password" hint="Use uma senha segura" label="Senha"
								...
								prepend-inner-icon="mdi-lock-outline" @click:append="show1 = !show1"></v-text-field>
						</v-col>

						<v-col cols="12">
							<span>CONFIRME SUA SENHA</span>
							<v-text-field v-model="password_confirm" label="Confirme sua senha"
								:type="show2 ? 'text' : 'password'" variant="outlined" clearable required
								:rules="[...passwordRules, passwordMatch]"
								:append-icon="show2 ? 'mdi-eye' : 'mdi-eye-off'" prepend-inner-icon="mdi-lock-outline"
								@click:append="show2 = !show2"></v-text-field>
						</v-col>

						<v-col cols="12" class="d-flex justify-center">
							<v-btn block color="#1356D3" size="large" :disabled="!isFormValid" @click="submit">
								CADASTRAR
							</v-btn>
						</v-col>
					</v-card>
